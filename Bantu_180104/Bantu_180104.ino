...
void loop(){
   if(fps > 50) {
    ...
    if (peak_L.available() && rms_L.available()) {
      leftPeak  = peak_L.readPeakToPeak() * 30; //use to be readPeakToPeak, and 15
...
//  showBArGragh(leftPeak,leftRMS);   
      ...
      if (BARKER_DISTANCE > currentDistance){
        if (state == 1){      
          if (noAction > 12000){  
//              analog_in = 10;
                analog_in = -0.0694*currentDistance+10.0+random(0,3);
              //noAction = 0;
              state = 2;
              if (currentPosition + analog_in < MIN_SWEEP){
                analog_in = -analog_in;
                }
//              noAction =- random(50,1000);
              }
        }  
        else if (state == 2){      
          if (noAction > 12000){  
//              analog_in = -10;
              ...
              if (currentPosition + analog_in > MAX_SWEEP){
                analog_in = -analog_in;
              }
//              noAction =- random(50,1000);
              }
  ...
  if (currentPosition > MAX_SWEEP ){
      ...
      stepper.setMaxSpeed(10000.0);  //50000.0
    ...
    else if(currentPosition < MIN_SWEEP){
      currentPosition = MIN_SWEEP ;  // may change speed here too??
      stepper.setMaxSpeed(50000.0);
      stepper.setSpeed(100000);
      stepper.setAcceleration(5000.0);
...
long  pingDistanceTwoPin (int pingPinOut ,int pingPinIn){
  ...
  digitalWrite(pingPinOut, LOW);
  duration = pulseIn(pingPinIn, HIGH, 500000);
  return duration;  
  }
...
*/
  ...
  currentDistanceNow = pingDistanceTwoPin(PING_PIN_01_TRIG, PING_PIN_01_ECHO); 
    ...
    if (currentDistanceNow < 100){

      currentDistanceNow = currentDistanceOld;
    }

    if (currentDistanceNow > 100){
      currentDistanceOld = currentDistanceNow;

    }

  
//  if (currentDistanceNow < 10){
//    Serial.println(currentDistanceNow);
//    currentDistanceNow = pingDistanceTwoPin(PING_PIN_01_TRIG, PING_PIN_01_ECHO);
//    Serial.println("2nd");
//    } 
